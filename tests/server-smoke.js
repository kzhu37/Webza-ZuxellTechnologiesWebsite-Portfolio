...
async function run() {
  try {
    ...
    const checks = [
      ...
      { name: "stylesheet HEAD", path: "/style.css", status: 200, type: "text/css", method: "HEAD" },
    ...
    for (const check of checks) {
      ...
      if (response.status !== check.status) {
        throw new Error(`${check.name} returned ${response.status}; expected ${check.status}`);
      }
      if (!contentType.startsWith(check.type)) {
        throw new Error(`${check.name} returned ${contentType}; expected ${check.type}`);
      }
      if (check.method === "HEAD" && (await response.text()).length !== 0) {
        throw new Error(`${check.name} returned a response body`);
      }
    }
